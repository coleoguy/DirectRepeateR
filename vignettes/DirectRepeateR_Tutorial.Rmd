...
- **outdir**: (Optional) A character string specifying the directory for the intermediate per-chromosome CSV files. Defaults to a unique temporary directory.

### Output
The intermediate chromosome CSV files will be saved out to the intermediate directory (`outdir`). These files will be named according to the sequence that those results correspond to and will have four columns:
...
file_path <- system.file("extdata", "C_elegans_chr1.fasta", package = "DirectRepeateR")
...
The `ConvertToGFF` function is designed to convert the output from the `GetRepeats` function into GFF3 (General Feature Format version 3). The function generates three corresponding GFF3 entries for each detected repeat: a `repeat_region` for the full length of the repeat, representing the region from the start of the first copy to the end of the second copy, and a `direct_repeat` for each of the two detected repeat copies. If a file path is supplied, a valid GFF3 file (starting with the `##gff-version 3` pragma) is written that can be loaded directly in genome browsers and tools such as IGV, rtracklayer, and bedtools.
...
- **path**: (Optional) A character string specifying the path for a GFF3 output file. If `NULL` (the default), no file is written and the GFF table is only returned.

### Output

The function returns a nine-column data frame in GFF3 layout, and optionally writes it to a valid GFF3 file.
...
# Plot repeat density; the ggplot objects are returned invisibly
# (one per chromosome) so they can be saved or modified later.
plots <- PlotRepeats(data = res, window_size = 200000, step_size = 200000)
```
